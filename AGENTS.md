...
- `src/ui/sessionExplorer.ts` implements the Crust activity-bar session browser webview, including refresh/new/open commands, filesystem watching, open-session highlighting across webview and terminal sessions, quick-pick restore, and default routing to the webview or terminal experience.
...
- `src/test/extension.test.ts` contains the VS Code integration/unit test suite for RPC guards/client behavior, slash command fallbacks and emulated commands, webview message validation, chat panel helpers, streaming rendering, session restore rendering (including compaction summaries), IDE context, tool utilities, usage formatting, path autocomplete, session history/session explorer integration, terminal-view wiring, settings, and static webview behavior.
