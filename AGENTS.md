...
- `src/ui/terminalView.ts` implements the opt-in terminal editor mode (`crust.chat.useTerminalViewByDefault`) that opens Pi's native TUI in a VS Code terminal tab and maintains a small IDE-context JSON file for the bundled Pi extension in `resources/pi/`.
